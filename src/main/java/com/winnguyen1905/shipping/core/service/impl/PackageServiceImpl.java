...
public class PackageServiceImpl implements PackageService {
  ...
  public PackageResponse createPackage(CreatePackageRequest request, TAccountRequest accountRequest) {
    ...
    EShipmentPackage packageEntity = EShipmentPackage.builder()
        ...
        .packageType(
            request.getPackageType() != null ? request.getPackageType() : ShippingConstants.DEFAULT_PACKAGE_TYPE)
        ...
        .isHazardous(
            request.getIsHazardous() != null ? request.getIsHazardous() : ShippingConstants.DEFAULT_IS_HAZARDOUS)
        .build();
  ...
  public PackageResponse updatePackage(Long id, CreatePackageRequest request, TAccountRequest accountRequest) {
    ...
    if (request.getTrackingNumber() != null &&
        (packageEntity.getTrackingNumber() == null
            || !packageEntity.getTrackingNumber().equals(request.getTrackingNumber()))
        &&
        packageRepository.existsByTrackingNumber(request.getTrackingNumber())) {
      throw new BadRequestException("Tracking number already exists: " + request.getTrackingNumber());
    }
    ...
    packageEntity
        .setPackageType(request.getPackageType() != null ? request.getPackageType() : packageEntity.getPackageType());
    ...
    packageEntity
        .setIsHazardous(request.getIsHazardous() != null ? request.getIsHazardous() : packageEntity.getIsHazardous());
  ...
  @Override
  @Transactional(readOnly = true)
  public List<PackageResponse.PackageValidationResult> bulkValidatePackages(List<Long> packageIds,
  ...
  @Override
  public PackageResponse.BulkPackageOperation bulkAssignTrackingNumbers(Map<Long, String> packageTrackingMap,
      TAccountRequest accountRequest) {
    log.info("Bulk assigning tracking numbers to {} packages for account: {}", packageTrackingMap.size(),
  ...
  @Override
  public List<PackageResponse> splitPackage(Long id, List<CreatePackageRequest> newPackageRequests,
      TAccountRequest accountRequest) {
    log.info("Splitting package ID: {} into {} packages for account: {}", id, newPackageRequests.size(),
        accountRequest.username());
  ...
  @Override
  public PackageResponse mergePackages(List<Long> packageIds, CreatePackageRequest mergedPackageRequest,
      TAccountRequest accountRequest) {
  ...
  private void validateCreatePackageRequest(CreatePackageRequest request) {
    ...
    if (request.getTrackingNumber() != null
        && !ShippingValidationUtils.isValidTrackingNumber(request.getTrackingNumber())) {
      throw new BadRequestException("Invalid tracking number format");
    }
  }
  ...
  private PackageResponse.PackageTrackingInfo mapToPackageTrackingInfo(EShipmentPackage packageEntity) {
    List<EShipmentTrackingEvent> events = trackingEventRepository
        .findByTrackingNumberOrderByEventTimestampDesc(packageEntity.getTrackingNumber());
    ...
    return PackageResponse.PackageTrackingInfo.builder()
        ...
        .estimatedDelivery(packageEntity.getShipment().getEstimatedDeliveryDate() != null
            ? packageEntity.getShipment().getEstimatedDeliveryDate().toString()
            : null)
  ...
  private PackageResponse.DimensionsResponse convertJsonToDimensions(String json) {
    ...
    try {
      CreatePackageRequest.DimensionsRequest dims = objectMapper.readValue(json,
          CreatePackageRequest.DimensionsRequest.class);
      BigDecimal volume = dims.getLength().multiply(dims.getWidth()).multiply(dims.getHeight());
      BigDecimal dimensionalWeight = ShippingValidationUtils.calculateDimensionalWeight(dims.getLength(),
