...
public interface ShippingWebhookRepository extends JpaRepository<EShippingWebhook, Long> {
    ...
    List<EShippingWebhook> findByReceivedAtBetween(@Param("startDate") Instant startDate, @Param("endDate") Instant endDate);
    
    @Query("SELECT sw FROM EShippingWebhook sw WHERE sw.processedAt BETWEEN :startDate AND :endDate")
    List<EShippingWebhook> findByProcessedAtBetween(@Param("startDate") Instant startDate, @Param("endDate") Instant endDate);
    
    @Query("SELECT sw FROM EShippingWebhook sw WHERE sw.carrier.carrierId = :carrierId AND sw.webhookType = :webhookType")
    List<EShippingWebhook> findByCarrierAndWebhookType(@Param("carrierId") Integer carrierId, @Param("webhookType") String webhookType);
